...
void LinkedList<T>::addFront(const T& val) {
    Node<T>* newNode = new Node<T>(val, head, NULL); // next -> point to head || prev -> point to NULL
    if (size == 0) head = tail = newNode;
    else {
        head->prev = newNode; // previous head's prev ptr point to the new head
        head = newNode;
    }
    size++;
}
...
void LinkedList<T>::addBack(const T& val) {
    ...
    Node<T>* newNode = new Node<T>(val, NULL, tail);// next -> point to NULL || prev -> point to tail
